...
function RenderComments({commentsArray, isOpen, postBComment, deleteBComment, blogId, author}){
                                        ...
                                        <Media>
                                            {
                                                ...
                                                ?
                                                <Button color='danger' className='mr-4' onClick={() => deleteBComment(comm._id)} >
...
class SingleBlog extends Component {
    ...
    addLike = async (likes) => {
        ...
        if(likes.length && likes.filter(uv => (uv.blog === this.props.blog._id && uv.user === this.props.auth.userId))[0]) {
            
            var v = likes.filter(uv => (uv.blog === this.props.blog._id && uv.user === this.props.auth.userId))[0];
    ...
    render(){
        ...
        return(
            <div>
                 <Card>
                    <CardHeader>
                        <CardTitle className='single-blog-heading'>{this.props.blog.heading}</CardTitle>
                    </CardHeader>
                    <CardBody>
                        <Row>
                            ...
                            <Col xs={12} md={9} lg={10}>
                                <Row>
                                    ...
                                    <Col xs={12}>
                                        <div className="editor__content" dangerouslySetInnerHTML={{ __html: this.props.blog.description }} />
                                    </Col>
                                </Row>
                            </Col>
                        </Row>
                        ...
                        <Row>
                            <ButtonGroup className='ml-4'>
                                <Button color='danger' onClick={ () => this.addLike(likes) }>
                                    <span className={ ilike ? 'fa fa-lg fa-heart mr-2' : 'fa fa-lg fa-heart-o mr-2'} />
                                    {likes.length}
                                </Button>
                                {/* <Button color='danger'>
                ...
                <Modal isOpen={this.state.shareModalOpen} toggle={() => this.onShareClicked()}>
                    ...
                    <ModalBody>
                        <FacebookShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'} quote={'Can you answer this ??'} hashtag={'#Poogle'}>
                            <FacebookIcon round={true}></FacebookIcon>
                        </FacebookShareButton>
                        <WhatsappShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'} separator={'\n'}>
                            <WhatsappIcon round={true}></WhatsappIcon>
                        </WhatsappShareButton>
                        <TelegramShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'}>
                            <TelegramIcon round></TelegramIcon>
                        </TelegramShareButton>
                        <LinkedinShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'} source={'WWW.poogle.com'}>
                            <LinkedinIcon round></LinkedinIcon>
                        </LinkedinShareButton>
                        <TwitterShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'} hashtags={'#Poogle'}>
                            <TwitterIcon round></TwitterIcon>
                        </TwitterShareButton>
                        <RedditShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} title={'Can you answer this ??'}>
                            <RedditIcon round></RedditIcon>
                        </RedditShareButton>
                        <PinterestShareButton url={'http://localhost:3000/blog-'+this.props.blog._id+'-'+this.props.blog.heading} description={'Can you answer this ??'} title={'Can you answer this ??'}>
